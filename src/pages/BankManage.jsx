...
export default function BankManage() {
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      if (depositForm.depositFrom === 'Others' && !depositForm.depositFromOther.trim()) {
        setErr('Please specify the other deposit source');
        return;
      }
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      if (withdrawForm.withdrawPurpose === 'Others' && !withdrawForm.withdrawPurposeOther.trim()) {
        setErr('Please specify the other withdrawal purpose');
        return;
      }
              ...
              <tbody>
                {transactions.map((t, i) => (
                  <tr key={t._id || i} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4 text-sm">
                      {t.type === 'deposit' ? (
                        <>From: {t.depositFrom}{t.depositFromOther ? ` (${t.depositFromOther})` : ''}</>
                      ) : (
                        <>For: {t.withdrawPurpose}{t.withdrawPurposeOther ? ` (${t.withdrawPurposeOther})` : ''}</>
                      )}
                    </td>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowDepositModal(false)}>
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl transform transition-all" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              {depositForm.depositFrom === 'Others' && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                  ...
                  <input
                    type="text"
                    value={depositForm.depositFromOther}
                    onChange={e => setDepositForm({ ...depositForm, depositFromOther: e.target.value })}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="Goods Buy">Goods Buy</option>
                  <option value="Others">Others</option>
                </select>
              </div>

              {withdrawForm.withdrawPurpose === 'Others' && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                  ...
                  <input
                    type="text"
                    value={withdrawForm.withdrawPurposeOther}
                    onChange={e => setWithdrawForm({ ...withdrawForm, withdrawPurposeOther: e.target.value })}
